...
public class ExcelItemReader implements ItemReader<PinCode> {
	...
	public PinCode read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
		...
		String pinCode = dataFormatter.formatCellValue(row.getCell(4));
		String city = dataFormatter.formatCellValue(row.getCell(3));
		String state = dataFormatter.formatCellValue(row.getCell(8));
